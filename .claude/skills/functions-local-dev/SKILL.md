...
2. Tell the user which values still need filling in: `AzureWebJobsStorage`, `ACS_CONNECTION_STRING`, `ACS_SENDER_ADDRESS`, `CONTACT_RECIPIENT_EMAIL`. Never invent placeholder-looking real credentials — leave this to the user.
3. From `functions/`, run `npm start` (this runs `func start` — requires Azure Functions Core Tools installed). `npm run build` is a no-op for this project; there's no compile step.
